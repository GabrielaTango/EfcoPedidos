...
namespace Delta3
{
    public class SQLH
    {
        public static string ToDateTime(DateTime dt)
        {
            ...
            if (dt.Year > 1)
            {
                ret = "CONVERT(DATETIME,'" + dt.ToString("dd/MM/yyyy") + "',103)";
            }
        ...
        public static string ToDouble(double doble)
        {
            string ret = "null";

            if (doble >= 0)
            {
                ret = "" + doble.ToString("0.00").Replace(",", ".") + "";
            }
            return ret;
        }
        ...
        internal static string ToBool(bool valor)
        {
            if (valor)
                return "1";
            return "0";
        ...
        {
            return $"(SELECT ID_{tabla} FROM {tabla} where {campo} = '{valor}')";
        }

        public static string TraerIdCampo(string tabla, string campoFrom, string campo, string valor)
        {
            return $"(SELECT {campoFrom} FROM {tabla} where {campo} = '{valor}')";
        }
    }
}